...
export const Sidebar: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full border-r-2 border-brutal-black bg-white" style={{ boxShadow: '4px 0 0 #141111', overflow: 'hidden' }}>
      {/* === Scrollable middle area === */}
      <div className="flex-1 sidebar-scroll" style={{ overflowY: 'scroll', overflowX: 'hidden', minWidth: 0, maxWidth: '100%' }}>

        {/* ── Section: 文件夹 ── */}
        <div className="border-b-2 border-brutal-black section-block">
          <div className="section-header flex items-center justify-between gap-2 px-3 py-2 bg-brutal-cream min-w-0">
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-1 h-4 bg-brutal-yellow border border-brutal-black flex-shrink-0" />
              <span className="font-bold text-xs uppercase text-black/80 truncate">文件夹</span>
              <span className="text-[10px] text-black/70 font-mono flex-shrink-0">{folders.length}</span>
            </div>
            <button
        ...
        <div className="border-b-2 border-brutal-black section-block">
          <div className="section-header flex items-center justify-between gap-2 px-3 py-2 bg-brutal-cream min-w-0">
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-1 h-4 bg-brutal-pink border border-brutal-black flex-shrink-0" />
              <span className="font-bold text-xs text-black/80 truncate">AI Agent</span>
              <span className="text-[10px] text-black/70 font-mono flex-shrink-0">{sortedAgents.length}</span>
            </div>
            <button
        ...
        <div className="border-b-2 border-brutal-black section-block">
          <div className="section-header flex items-center justify-between gap-2 px-3 py-2 bg-brutal-cream min-w-0">
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-1 h-4 bg-brutal-lavender border border-brutal-black flex-shrink-0" />
              <span className="font-bold text-xs uppercase text-black/80 truncate">历史</span>
              <span className="text-[10px] text-black/70 font-mono flex-shrink-0">{sessions.length}</span>
            </div>
            <button
      ...
      {/* === Fixed bottom bar === */}
      <div className="p-3 border-t-2 border-brutal-black space-y-2 flex-shrink-0 overflow-hidden">
        ...
        <div className="flex gap-2 min-w-0">
          <button
            ...
            className="flex-1 min-w-0 flex items-center justify-center gap-1.5 px-2 py-2 border-2 border-brutal-black bg-white hover:bg-brutal-yellow shadow-brutal-sm hover:shadow-brutal hover:-translate-x-[1px] hover:-translate-y-[1px] active:shadow-none active:translate-x-0 active:translate-y-0 transition-all duration-150 ease-out overflow-hidden"
            title="Token 用量"
          >
            <Coins size={14} />
            <span className="text-xs font-mono font-bold truncate">{sessionTotal > 0 ? (sessionTotal >= 1000 ? `${(sessionTotal / 1000).toFixed(1)}k` : sessionTotal) : '0'}</span>
          </button>
          <button
          ...
          >
            <Settings size={14} />
            <span className="text-xs font-bold truncate">设置</span>
          </button>
        </div>
      </div>
    </div>
